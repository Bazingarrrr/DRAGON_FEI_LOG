...
void calc_stage_title(){
	if(stage_count==stage_title_count[0]){//今が表示開始時なら
		stage_title.flag=1;
		stage_title.img = LoadGraph("../dat/img/board/subject0.png");
	}

	if (stage_count == stage_title_count[1]) {//今が表示開始時なら
		stage_title.flag = 1;
		stage_title.img = LoadGraph("../dat/img/board/subject1.png");
	}

	if (stage_count == stage_title_count[2]) {//今が表示開始時なら
		stage_title.flag = 1;
		stage_title.img = LoadGraph("../dat/img/board/subject2.png");
	}

	if (stage_count == stage_title_count[3]) {//今が表示開始時なら
		stage_title.flag = 1;
		stage_title.img = LoadGraph("../dat/img/board/subject3.png");
	}

	if (stage_count == stage_title_count[4]) {//今が表示開始時なら
		stage_title.flag = 1;
		stage_title.img = LoadGraph("../dat/img/board/subject4.png");
	}
